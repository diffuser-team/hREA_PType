...
export default (
  enabledVFModules: VfModule[] = DEFAULT_VF_MODULES,
  ...
  const readRevision = mapZomeFn<ByRevision, ProposalResponse>(
    dnaConfig,
    conductorUri,
    "proposal",
    "proposal",
  ...
  const readProposedTo = mapZomeFn<ReadParams, ProposedToResponse>(
    dnaConfig,
    conductorUri,
    "proposal",
    "proposed_to",
  ...
  const readProposedIntent = mapZomeFn<ReadParams, ProposedIntentResponse>(
    dnaConfig,
    conductorUri,
    "proposal",
    "proposed_intent",
  ...
  return Object.assign(
    {
      publishes: async (record: {
        publishes: ProposedIntentAddress[];
      }): Promise<ProposedIntent[]> => {
        return (
          await Promise.all(
            (record.publishes || []).map((address) =>
              readProposedIntent({ address })
            )
          )
        ).map(extractProposedIntent);
      },
    },
    hasAgent
      ? {
          publishedTo: async (record: {
            ...
            return (
              await Promise.all(
                (record.publishedTo || []).map((address) =>
                  readProposedTo({ address })
                )
              )
            ).map(extractProposedTo);
          },
          inScopeOf: async (record: {
            inScopeOf: AgentAddress[];
          }): Promise<AccountingScope[]> => {
            return await Promise.all(
              (record.inScopeOf || []).map((address) =>
                readAgent(record, { address })
              )
            );
          },
        }
    ...
    hasHistory
      ? {
          revision: async (
            record: Proposal,
            args: { revisionId: AddressableIdentifier }
          ): Promise<Proposal> => {
